...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # save image sample to check
        # sample = image
        # cv2.imwrite('traw_image.jpg', sample)
        ...
        # #sample = sample
        # cv2.imwrite('transformed_image.jpg', sample)
...
##
    def load_cutmix_image_and_boxes(self, index, imsize=1024):
        ...
        # 위에도 트랜스폼에서 같은 코드가 있지만 그렇게 하면 컷믹스 데이터가 트랜스폼 무한 루프에 빠진다.
        # 빈 리스트 표현법으로 lenㅡㄴ 비효율적임 is not x: print('empty') 참고
